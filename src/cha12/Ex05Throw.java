...
// ★★ throw문 사용 이유 : 컴파일, 런타임 오류 외 사용자가 추가적으로 조건(if문)을 걸고, 해당 조건에 일치하지 않는 경우 throw를 통해 오류를 강제로 발동시킴 ★★
//★ throw 메커니즘 : ①조건(if문)에 맞지 않을 경우 throw가 오류를 발생시킴(오류객체가 생성됨) → ②thorw가 오류를 받은 후 해당 예외클래스(java가 아닌 직접 만든 예외클래스) 생성자 호출(호출 가능한 생성자가 반드시 있어야함) → ③ catch블럭에서 해당 오류객체를 받은 후 실행(catch블럭에 해당 예외클래스가 구현되어 있을 경우에만) 
	// DB_ID와user.getId()가 일치하지 않더라도 컴파일 및 런타임에러는 발생하지 않으나, throw가 오류를 강제로 발동시키므로, catch문이 실행됨
