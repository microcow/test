...
public class Ex01FunctionalInterface {
	public static void main(String[] args) {
		...
		// ★ 람다식에서는 return을 적지 않아도 된다.
		// ★ Supplier 인터페이스는 파라미터는 필요 없고 return(Integer 타입)을 필요로 하는 메소드가 람다식으로 필요로 할 때 사용
		
		/* ★★ 익명클래스에서 new Supplier와 같이 인스턴스를 생성(형체는 없는) 후 오버라이딩 하여 as라는 인스턴스 변수에 저장하고 있듯,
		   람다식에서도 인스턴스를 생성하지만, new(인스턴스 생성)코드가 생략되었으며, 오버라이딩(람다식)한 인스턴스를 s에 저장하는 방식이다.
		   ★ 즉, 람다식으로 s에 직접 오버라이딩하는게 아닌 람다식으로 오버라이딩 한 인스턴스(형체는 없는)를 s에 저장하고 있다. 
		   	 따라서, s.추상메소드 호출 시 오버라이딩 한 람다식이 실행된다.*/
