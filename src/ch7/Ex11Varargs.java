...
class Book12 {
	...
	static void printInfo(Book12 book) { 
		...
		// tip) 클래식 메소드에 인스턴스를 던질 땐 메소드 앞에 클래스이름을 붙여서 호출(예시, 클래스이름.메소드이름(던질인스턴스이름)), 그냥 메소드에 인스턴스를 던질 땐 메소드 앞에 인스턴스 이름(예시, 호출인스턴스이름.메소드이름(던질인스턴스이름) 단, 호출하는인스턴스의 클래스와 호출될메소드가 있는 클래스가 일치(상속)해야하며, 던져지는 인스턴스클래스와 받는 파라미터의 클래스도 일치해야한다.)
		// tip2) 참조변수(인스턴스)를 받을 땐(클래식 메소드) 파라미터 앞에 클래스이름, 그냥 인스턴스 변수를 받을 땐(그냥 메소드) 파라미터 앞에 타입(float, int 등등)
