...
public class Ex06ClassMethod {
	public static void main(String[] args) {
		...
		// 클래스 메서드는 클래스이름.메서드이름 으로 호출 (인스턴수 변수.메서드로 호출X)
...
class Book5 {
	...
	static int generateBookNo() {
		...
		/*  클래스 메서드(스태틱 메서드)는 클래스 변수(스태틱 변수)와 동일하게 타입 앞에 static이 붙으며,
		 	클래스 메서드(스태틱 메서드)내에서 인스턴스 변수사용하려면 인스턴스 자체를 파라미터로 받아야한다. (*ch7 Ex11)
		 ...
		 // 따라서, 클래스 메서드(스태틱 메서드)에서는 this.title 등 this.변수이름을 사용할 수 없다
		// 클래스 메서드는 특정 인스턴스가 생성되기 이전에 실행(저장)되기 때문에, 특정 인스턴스의 변수를 사용할 수 없는 것
