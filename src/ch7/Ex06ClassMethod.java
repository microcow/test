...
class Book5 {
	...
	static int generateBookNo() {
		...
		/*  클래스 메서드(스태틱 메서드)는 클래스 변수(스태틱 변수)와 동일하게 타입 앞에 static이 붙으며,
		 	클래스 메서드(스태틱 메서드) 내에서는 인스턴스 변수를 사용할 수 없다 (여기선 title, bookNo)
		 	왜냐하면 클래스 메서드(스태틱 메서드)는 인스턴스의 이름으로 호출하는 것이 아닌 클래스의 이름으로 호출하기 때문
		 	또한, 클래스 메서드(스태틱 메서드)에서는 클래스 변수(스태틱 변수)는 사용 가능하지만, 인스턴스 변수는 사용이 불가하다. 
		 	but, 메서드 내에서만 생성하는 '지역변수'는 사용 가능	*/
		// 추가적으로, this.title과 같은 변수도 사용할 수 없음
