...
public class Ex01Upcasting {
	public static void main(String[] args) {
		...
		// 하지만, book은 EBook클래스의 인스턴스 변수 구조를 가지고 있지만, 부모클래스 소속이므로 자식 클래스의 메소드나 인스턴스 변수를 사용할 수 없다. (* ch8 Ex01)
		// 질문 1 - 자식클래스의 인스턴스 변수도 사용 불가한가? (EBook 클래스 구조를 가지고있는데?)
		
		/* 문제 1. book.setFontColor("블랙") 코드에서 컴파일 오류가 나지 않도록 수정해 주세요.
		book.setFontColor("블랙");
		System.out.println(book.getFontColor()); */
		// → book클래스는 EBook클래스 구조를 가지고 있으나 부모클래스이기에 자식클래스의 메소드인 setFontColor를 사용할 수 없다 따라서, setFountColor 메소드를 부모클래스에서 생성하고 자식클래스에 있는 메소드는 오버라이딩 시켜야한다
